...
function drawBar(ctx, w, h, pad, labels, values, color, offset = 0, seriesCount = 1) {
...
function drawLine(ctx, w, h, pad, labels, values, color) {
  const maxVal = Math.max(...values, 1);
  const maxY = getNiceMax(maxVal);
  drawAxes(ctx, w, h, pad, maxY, labels);
...
function drawDonut(ctx, w, h, values, color) {
  ...
  document.querySelectorAll("canvas[data-chart]").forEach((canvas) => {
    ...
    if (chartType === "bar") {
      if (s2) {
        ...
        drawBar(ctx, w, h, pad, s1.labels, s1.values, "rgba(0, 206, 201, 0.85)", 0, 1);
      }
      return;
    }

    if (chartType === "line") {
      drawLine(ctx, w, h, pad, s1.labels, s1.values, "#1e90ff");
      return;
    }

    if (chartType === "donut") {
      drawDonut(ctx, w, h, s1.values, "#00b894");
    }
  });
}
